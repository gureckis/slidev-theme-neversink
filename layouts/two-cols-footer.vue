...
const alignment = computed(() => {
  ...
  return { f: compute_alignment(parts[0]), l: compute_alignment(parts[1]), r: compute_alignment(parts[2]) }
})
...
<template>
  ...
  >
    ...
    <p>
      In addition you can specify "slots" of the page with <code>:: footer ::</code>, <code>:: left ::</code>, and
      <code>:: right::</code>. Footer is optional.
    </p>
    <p>
      The <code>align</code> parameter determines how the columns look. The notation is for example
      <code>align: c-cm-cm</code>. The first part is for the footer, the second for the left column, and the third part
  ...
  <div v-else :class="flexclass">
    ...
    <div v-if="$slots.footer" class="footer" :class="alignment.f">
      <slot name="footer" />
    </div>
...
.two-cols-footer {
  ...
  grid-template-rows: auto 1fr; /* footer and content */
}
